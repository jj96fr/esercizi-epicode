...
const Current = () => {
  ...
  const baseEndpointWeather =
    "https://api.openweathermap.org/data/2.5/weather?appid=563305e5f5efd440d89a1e8bbc5871ed&units=metric";
  const baseEndpointLocation =
    "http://api.openweathermap.org/geo/1.0/direct?limit=1&appid=563305e5f5efd440d89a1e8bbc5871ed&q=";
      ...
      <Row className="d-flex justify-content-start ps-5 w-100 pt-5 ">
        <div className="d-flex justify-content-center">
          <Col className="d-flex flex-column ms-5" xs={4}>
            <h2 className="text-dark fw-bold fs-1 text-start mb-3 mt-5 ">
              Today
            </h2>
            {isLoading && !isError && <Loading />}
            {!isLoading && isError && <Error />}
            {localWeather && !isLoading && !isError && (
              <Col className="myCard mt-3" xs={10}>
